#!/bin/bash
# ============================================================================
# Agent Pipeline Orchestrator — React Native
# ============================================================================
# Usage:
...
#   - Agent files in shared agents/ and react-native/agents/
# ============================================================================
...
HEADER
    ...
    # File structure (3 levels deep, ignore noise)
    echo -e "\n## Project Structure\n\`\`\`" >> "$snapshot"
    ...
    find . -type f \( -name "schema.*" -o -name "*.model.*" -o -name "*.entity.*" -o -path "*/models/*" -o -path "*/schema/*" -o -path "*/database/*" \) \
        -not -path '*/node_modules/*' \
        -not -path '*/android/*' \
        -not -path '*/ios/*' \
        -exec echo "// === {} ===" \; \
    ...
    echo '```typescript' >> "$snapshot"
    find . -type f \( -path "*/api/*" -o -path "*/services/*" \) -name "*.ts" \
        -not -path '*/node_modules/*' \
        | sort \
        | while read -r f; do
            echo "// === $f ==="
...
- Scan the codebase (src/, tests/) to verify what files exist
...
main() {
    ...
    log_ok "All agent files found (shared + React Native)"
